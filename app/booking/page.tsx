...
function BookingForm() {
    ...
    return (
      <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 16px' }}>
        <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ textAlign: 'center', maxWidth: 480 }}>
          <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'linear-gradient(135deg, var(--saffron), var(--gold))', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 22px', fontSize: 32, color: 'white' }}>✓</div>
          <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: 'clamp(24px,4vw,32px)', fontWeight: 700, color: 'var(--deep)', marginBottom: 14 }}>Booking Request Sent!</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: 15, lineHeight: 1.75, marginBottom: 28 }}>
            Thank you, <strong>{form.name}</strong>! Your booking for <strong>{form.from} → {form.to}</strong> on <strong>{form.travelDate}</strong> has been sent successfully; email. We'll confirm within 30 minutes.
          </p>
          {couponResult?.valid && (
            <div style={{ background: 'rgba(45,122,79,0.09)', border: '1px solid rgba(45,122,79,0.25)', borderRadius: 12, padding: '11px 18px', marginBottom: 22, color: 'var(--green)', fontWeight: 600, fontSize: 14 }}>
  ...
  return (
    <form onSubmit={handleSubmit} autoComplete="off" style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(32px,5vw,56px) clamp(16px,3vw,24px)' }}>
      ...
      <div style={{ background: 'white', borderRadius: 20, padding: 'clamp(20px,4vw,36px)', boxShadow: '0 8px 32px rgba(26,15,5,0.07)', border: '1px solid rgba(232,101,26,0.07)', display: 'flex', flexDirection: 'column', gap: 0 }}>

        {/* Personal Info */}
        <div style={{ marginBottom: 28 }}>
          <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 17, color: 'var(--deep)', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
            <User size={16} color="var(--saffron)" /> Personal Details
          </h3>
        ...
        {/* Trip Details */}
        <div style={{ marginBottom: 28, paddingTop: 24, borderTop: '1px solid rgba(232,101,26,0.08)' }}>
          <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 17, color: 'var(--deep)', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
            <MapPin size={16} color="var(--saffron)" /> Trip Details
          </h3>
        ...
        {estimatedFare > 0 && (
          <div style={{ marginBottom: 28, paddingTop: 24, borderTop: '1px solid rgba(232,101,26,0.08)' }}>
            <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 17, color: 'var(--deep)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Tag size={16} color="var(--saffron)" /> Coupon Code
            </h3>
            ...
            <div style={{ marginTop: 14, background: 'rgba(232,101,26,0.05)', borderRadius: 10, padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
              <span style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                Estimated Fare: <strong style={{ color: couponResult?.valid ? 'var(--text-light)' : 'var(--deep)', textDecoration: couponResult?.valid ? 'line-through' : 'none' }}>₹{estimatedFare.toLocaleString()}</strong>
              </span>
              <span style={{ fontFamily: 'Playfair Display, serif', fontSize: 'clamp(22px,4vw,28px)', fontWeight: 700, color: couponResult?.valid ? 'var(--green)' : 'var(--saffron)' }}>₹{finalFare.toLocaleString()}</span>
            </div>
          </div>
        )}
        ...
        <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-light)', marginTop: 10, lineHeight: 1.5 }}>
