...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #071523 0%, #0B2344 50%, #071523 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden' }}>
      {/* Background decoration */}
      <div style={{ position: 'absolute', top: '20%', left: '10%', width: 300, height: 300, borderRadius: '50%', background: 'radial-gradient(circle, rgba(11,35,68,0.06) 0%, transparent 70%)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: '20%', right: '10%', width: 400, height: 400, borderRadius: '50%', background: 'radial-gradient(circle, rgba(212,175,55,0.04) 0%, transparent 70%)', pointerEvents: 'none' }} />

      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        style={{ width: '100%', maxWidth: 420 }}>
        {/* Logo */}
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <img src="/Dark Mode Logo.png" alt="R" style={{ width: 72, height: 72, objectFit: 'contain', borderRadius: '50%', margin: '0 auto 20px', boxShadow: '0 8px 32px rgba(11,35,68,0.4)' }} />
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, padding: 36, backdropFilter: 'blur(20px)' }}>
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input value={username} onChange={e => setUsername(e.target.value)} placeholder="Admin username" required
                  ...
                  onFocus={e => (e.target as HTMLElement).style.borderColor = '#D4AF37'}
            ...
            <button type="submit" disabled={loading}
              style={{ padding: '15px', background: 'linear-gradient(135deg, #0B2344, #071523)', border: 'none', borderRadius: 12, color: 'white', fontSize: 16, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'DM Sans, sans-serif', boxShadow: '0 4px 20px rgba(11,35,68,0.4)', transition: 'all 0.3s', opacity: loading ? 0.7 : 1 }}>
              {loading ? 'Signing in...' : 'Sign In to Admin'}
            </button>
          </form>

          <div style={{ marginTop: 24, padding: '14px', background: 'rgba(11,35,68,0.06)', borderRadius: 10, border: '1px solid rgba(11,35,68,0.12)' }}>
