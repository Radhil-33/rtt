      ...
      <div className="container">
        <div className="why-grid">
          ...
          <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.55 }}>
            ...
            <p style={{ fontSize: 15, color: 'var(--text-body)', lineHeight: 1.8, marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
              {[
                ...
                <div key={label}>
                  <div style={{ fontFamily: 'Playfair Display, serif', fontSize: 'clamp(24px,4vw,32px)', fontWeight: 700, color: '#D4AF37', lineHeight: 1 }}>{num}</div>
                  <div style={{ fontSize: 12, color: 'var(--text-body)', fontWeight: 500, marginTop: 4 }}>{label}</div>
                </div>
              ))}
            </div>
          </motion.div>
            ...
            {features.map((feat, i) => {
              ...
              return (
                <motion.div key={feat.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.38, delay: i * 0.07 }}>
                  <div style={{ background: 'var(--bg-surface)', borderRadius: 14, padding: '18px 16px', border: '1px solid var(--border-light)', transition: 'all 0.28s', height: '100%' }}
                    onMouseEnter={e => {
                      (e.currentTarget as HTMLElement).style.borderColor = 'var(--border-mid)';
                      (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)';
                      (e.currentTarget as HTMLElement).style.boxShadow = 'var(--shadow-navy)';
                    }}
                    onMouseLeave={e => {
                    ...
                    <div style={{ width: 40, height: 40, borderRadius: 10, background: isDark ? 'linear-gradient(135deg, rgba(212,175,55,0.12), rgba(212,175,55,0.06))' : 'linear-gradient(135deg, rgba(11,35,68,0.08), rgba(212,175,55,0.06))', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
                      <Icon size={19} color={isDark ? "#D4AF37" : "#0B2344"} />
                    </div>
                    <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: 14, fontWeight: 700, color: 'var(--text-heading)', marginBottom: 5 }}>{feat.title}</h3>
                    <p style={{ fontSize: 13, color: 'var(--text-body)', lineHeight: 1.6 }}>{feat.desc}</p>
                  </div>
                </motion.div>
              );
            })}
