...
export default function QuickBookCTA() {
  return (
    <section className="section" style={{ background: 'var(--bg-page-alt)' }}>
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.55 }}
          style={{ background: 'linear-gradient(135deg, #0B2344, #071523)', borderRadius: 24, padding: 'clamp(36px,6vw,68px) clamp(20px,5vw,64px)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -50, right: -50, width: 220, height: 220, borderRadius: '50%', background: 'radial-gradient(circle,rgba(212,175,55,0.1) 0%,transparent 70%)', pointerEvents: 'none' }} />
          <div style={{ position: 'relative', zIndex: 1 }}>
            <span className="section-tag" style={{ background: 'rgba(212,175,55,0.18)', color: '#D4AF37' }}>Ready to Travel?</span>
            ...
            <p style={{ color: 'rgba(248,246,240,0.68)', fontSize: 'clamp(14px,2vw,17px)', maxWidth: 500, margin: '0 auto 36px', lineHeight: 1.7 }}>
              Call us for instant confirmation, or book online and we&apos;ll reach you within 30 minutes.
            </p>
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              <a href={`tel:${PHONE}`} className="btn-primary" style={{ fontSize: 'clamp(14px,1.8vw,16px)', padding: 'clamp(13px,2vw,16px) clamp(18px,3vw,32px)', background: 'linear-gradient(135deg, #D4AF37, #B8951F)', color: '#0B2344', fontWeight: 700 }}>
