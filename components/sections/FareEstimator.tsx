...
export default function FareEstimator() {
  ...
  return (
    <section id="fare-estimator" className="section" style={{ background: 'linear-gradient(180deg, var(--bg-page), var(--bg-page-alt))' }}>
      <div className="container">
        ...
        <div style={{ background: 'var(--bg-surface)', borderRadius: 20, padding: 'clamp(20px,4vw,40px)', boxShadow: 'var(--shadow-navy)', border: '1px solid var(--border-light)', maxWidth: 860, margin: '0 auto' }}>
          {/* Row 1 */}
          ...
          <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
            {(['one-way', 'round'] as const).map(t => (
              <button key={t} onClick={() => { setTripType(t); setCalculated(false); }}
                style={{ padding: '9px 20px', borderRadius: 50, border: '2px solid', borderColor: tripType === t ? (isDark ? '#D4AF37' : 'var(--text-heading)') : 'var(--border-mid)', background: tripType === t ? (isDark ? 'rgba(212,175,55,0.08)' : 'var(--tag-bg)') : 'transparent', color: tripType === t ? (isDark ? '#D4AF37' : 'var(--text-heading)') : 'var(--text-body)', fontWeight: 600, cursor: 'pointer', fontSize: 14, fontFamily: 'DM Sans, sans-serif', transition: 'all 0.2s', flex: '1 1 auto', maxWidth: 160 }}>
          ...
          <AnimatePresence>
            {calculated && estimate && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ marginTop: 24, overflow: 'hidden' }}>
                <div style={{ background: 'linear-gradient(135deg, var(--tag-bg), rgba(212,175,55,0.04))', border: '1px solid var(--border-mid)', borderRadius: 14, padding: 'clamp(16px,3vw,24px)' }}>
                  {/* Route + fare */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 14, marginBottom: 18 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                        <MapPin size={15} color={isDark ? "#D4AF37" : "#0B2344"} />
                        <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-heading)' }}>{from} <ArrowRight size={12} style={{ display: 'inline' }} /> {to} · {estimate.distance} km</span>
                      </div>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 10, textAlign: 'center' }}>* Estimate only. Actual fare may vary based on tolls and waiting time.</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
